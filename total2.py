...
NAVER_CLIENT_ID = "5oYO4svqBKFHwAx3Uc7T"
NAVER_CLIENT_SECRET = "7jpulJWz20"
...
tofu_burrito_questions = [
   "두부 브리또 어디서 사는 게 가장 좋을까요?",
   "두부 브리또 브랜드별로 비교해주세요.",
   "두부 브리또 가성비 좋은 제품은 무엇인가요?"
]
default_queries_map = {
    "쇼핑": tofu_burrito_questions[0], # 쇼핑 탭 기본 질문
    "블로그": "두부 브리또 집에서 만들기 후기가 궁금해요.",
    "뉴스": "두부 브리또 트렌드와 건강식 시장 동향은?"
}
...
# 검색 입력 필드 도움말 텍스트
help_texts = {
    "쇼핑": "두부 브리또 구매 관련 질문을 클릭하거나 직접 검색어를 입력하세요.",
    "블로그": "블로그 관련 검색어를 입력하세요. (예: 두부 브리또 만들기 후기)",
    "뉴스": "뉴스 관련 검색어를 입력하세요. (예: 건강식품 트렌드)"
}
...
if active_source_type == "쇼핑":
    st.markdown("👇 **두부 브리또 관련 추천 질문을 선택해보세요!**")
    cols = st.columns(len(tofu_burrito_questions))
    for i, q_text in enumerate(tofu_burrito_questions):
        if cols[i].button(q_text, key=f"tofu_q_btn_{i}"):
